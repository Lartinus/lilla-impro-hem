-- Create debug function to see what's happening
CREATE OR REPLACE FUNCTION public.debug_delete_participant(table_name text, participant_email text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
AS $function$
...
BEGIN
  ...
  INTO found_records USING lower(participant_email);
  ...
  debug_info := jsonb_build_object(
    ...
    'search_email', lower(participant_email)
  );
