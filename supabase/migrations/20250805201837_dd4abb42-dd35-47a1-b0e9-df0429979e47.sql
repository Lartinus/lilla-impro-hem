...
ADD COLUMN IF NOT EXISTS scanned_tickets integer DEFAULT 0,
ADD COLUMN IF NOT EXISTS partial_scan boolean DEFAULT false;
...
WHERE scanned_status = true AND scanned_tickets = 0;
...
-- Drop and recreate get_ticket_by_qr to include new fields
DROP FUNCTION IF EXISTS public.get_ticket_by_qr(text);

CREATE FUNCTION public.get_ticket_by_qr(qr_data_param text)
RETURNS TABLE(
  id uuid, 
  show_title text, 
  show_date text, 
