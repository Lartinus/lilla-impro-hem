-- Create a simplified function that works with the admin context
-- This function will work when called from the admin interface
CREATE OR REPLACE FUNCTION public.get_course_booking_count(table_name text)
RETURNS integer
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO public
AS $function$
DECLARE
  booking_count INTEGER := 0;
BEGIN
  -- Check if table exists first
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.tables 
    WHERE table_schema = 'public' 
    AND table_name = $1
  ) THEN
    RETURN 0;
  END IF;
  
  -- Use dynamic SQL to count rows - this should work because the function is SECURITY DEFINER
  -- and will run with the permissions of the function creator
  EXECUTE format('SELECT COUNT(*) FROM public.%I', table_name) INTO booking_count;
  
  RETURN COALESCE(booking_count, 0);
EXCEPTION
  WHEN OTHERS THEN
    -- Log error for debugging
    RAISE NOTICE 'Error in get_course_booking_count for table %: %', table_name, SQLERRM;
    RETURN 0;
END;
$function$;