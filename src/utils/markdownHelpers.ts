        ...
        <div className="text-left space-y-6">
          {mainInfo?.info && (
            <div 
              ...
              dangerouslySetInnerHTML={{ __html: convertMarkdownToHtml(String(mainInfo.info)) }}
            />
          )}
          ...
          {mainInfo?.redbox && (
            <div className="bg-red-700 p-6 rounded-none relative text-white">
              <div 
                ...
                style={{ lineHeight: '1.3' }}
                dangerouslySetInnerHTML={{ __html: convertMarkdownToHtmlForRedBox(String(mainInfo.redbox)) }}
              />
            </div>
          )}
          ...
          {mainInfo?.infoAfterRedbox && (
            <div 
              className="space-y-6 text-gray-700 leading-relaxed text-base"
              style={{ lineHeight: '1.3' }}
              dangerouslySetInnerHTML={{ __html: convertMarkdownToHtml(String(mainInfo.infoAfterRedbox)) }}
            />
          )}
        </div>
      </div>
