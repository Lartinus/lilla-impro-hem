...
const About = () => {
  ...
  return <div className="min-h-screen flex flex-col bg-gradient-to-br from-theatre-primary via-theatre-secondary to-theatre-tertiary text-theatre-light font-satoshi">
      ...
      {/* Main Content */}
      <section className="px-0.5 md:px-4 mt-20 flex-1 py-0 my-[60px]">
        <div className="space-y-8 border-4 border-white p-6 md:p-6 lg:p-12 bg-white rounded-none mx-3 md:mx-0 md:max-w-4xl md:mx-auto py-[9px]">
          
          {/* Main info content with markdown conversion */}
          {content?.info && <div className="space-y-6 text-gray-700 leading-relaxed text-base" style={{
          lineHeight: '1.3'
        }} dangerouslySetInnerHTML={{
          __html: convertMarkdownToHtml(content.info)
        }} />}

          {/* Performers section */}
          {performers && performers.length > 0 && <PerformersSection performers={performers} />}
