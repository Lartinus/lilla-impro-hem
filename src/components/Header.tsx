...
<header className="fixed top-0 left-0 right-0 z-50 bg-theatre-tertiary text-theatre-light backdrop-blur-md font-satoshi">
  ...
  className="hidden lg:flex relative top-1 lg:top-2 text-theatre-light font-satoshi font-semibold space-x-12 text-lg">
          ...
          <Button
            variant="ghost"
            className="flex lg:hidden text-theatre-light hover:bg-theatre-light/20 font-satoshi p-4 [&_svg]:!size-auto"
            onClick={() => setIsMenuOpen(!isMenuOpen)}
            aria-label={isMenuOpen ? 'Stäng meny' : 'Öppna meny'}
          >
            {isMenuOpen ? <X size={144} strokeWidth={4} className="text-theatre-light w-12 h-12" /> : <Menu size={32} strokeWidth={4} className="text-theatre-light w-12 h-12" />}
          </Button>
        </div>
