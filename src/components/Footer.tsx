  ...
  return (
    <footer className="bg-footer-black text-white py-8 md:py-12">
      <div className="container mx-auto px-6 lg:px-8">
        <div className="flex flex-col-reverse md:flex-row md:justify-between md:items-start space-y-reverse space-y-8 md:space-y-0">
          
          {/* #2 Länkdelen - först på mobil */}
          <nav className="flex flex-col space-y-2 md:text-right md:order-2">
            {footerNavItems.map(({ to, label }) => (
              <Link
                key={to + label}
                to={to}
                className="font-satoshi text-sm uppercase tracking-wider hover:text-white/80 transition-colors underline"
              >
                {label}
              </Link>
            ))}
          ...
          {/* #1 Logotyp-delen - andra på mobil */}
          <div className="md:max-w-md md:order-1">
