...
const SmallCourseCard = ({ course }: SmallCourseCardProps) => {
  ...
  const remainingSpots =
    course.max_participants && course.currentParticipants !== undefined
  ...
  const normalizedPrice =
    typeof course.price === 'number' && course.price >= 10000
      ? Math.round(course.price / 100)
      : course.price;
  const normalizedDiscount =
    typeof course.discount_price === 'number' && course.discount_price >= 10000
      ? Math.round(course.discount_price / 100)
  ...
  return (
    <Card className="bg-[#E7E7E7] transition-all duration-300 border-none shadow-none rounded-none">
      {/* Viktigt: ingen flex-1 här */}
      <CardContent className="p-4 flex flex-col">
        {/* Titel + ev. undertitel + avdelare */}
        <div>
          ...
          <div className="pt-1 my-1">
            <div className="flex items-center justify-center relative mb-1">
              ...
              {(() => {
                const remainingSpotsInner =
                  course.max_participants && course.currentParticipants !== undefined
                    ? course.max_participants - course.currentParticipants
                    : null;
                const isSoldOutInner =
                  remainingSpotsInner !== null && remainingSpotsInner <= 0;
                const showFewSpotsWarning =
                  remainingSpotsInner !== null &&
                  remainingSpotsInner <= 5 &&
                  remainingSpotsInner > 0;

                return isSoldOutInner || showFewSpotsWarning ? (
                  <div className="relative bg-[#E7E7E7] px-2">
                    <div className="inline-flex items-center justify-center w-[120px] h-[22px] text-[12px] rounded-full border-2 font-rajdhani font-medium bg-primary border-primary text-white">
                      {isSoldOutInner ? 'Fullbokad!' : 'Få platser kvar'}
                    </div>
                  </div>
                ) : null;
              })()}
            ...
            <div className="min-h-[24px]">
              <div
                ...
                dangerouslySetInnerHTML={{
                  __html: convertMarkdownToHtml(course.description || ''),
                }}
              />
            </div>
          </div>
